...
def main():
    ...
    bayes_rgb_model.fit(X_train, np.ravel(y_train))
    
    bayes_convert_model = make_pipeline(FunctionTransformer(color.rgb2lab), GaussianNB())
    bayes_convert_model.fit(X_train, np.ravel(y_train))

    knn_rgb_model = KNeighborsClassifier(n_neighbors=30)
    knn_rgb_model.fit(X_train, y_train)
    knn_convert_model = make_pipeline(FunctionTransformer(color.rgb2lab), KNeighborsClassifier(n_neighbors=30))
    ...
    rf_convert_model = make_pipeline(FunctionTransformer(color.rgb2lab), RandomForestClassifier(n_estimators=200, max_depth=10))
