...
def main(infile):
    ...
    model_rgb.fit(X_train, np.ravel(y_train))
    print(model_rgb.score(X_train, np.ravel(y_train)))

    # TODO: build model_lab to predict y from X by converting to LAB colour first.
    # TODO: print model_lab's accuracy score
    
    model_lab = make_pipeline(FunctionTransformer(color.rgb2lab), GaussianNB())
    model_lab.fit(X_train, np.ravel(y_train))
    print(model_lab.score(X_train, np.ravel(y_train)))
